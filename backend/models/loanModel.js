...
const LoanModel = {
  async createLoan({ memberID, userID, amount, reason, termMonths, interestRate, paymentFrequency }) {
    const totalAmount = amount * (1 + interestRate); // Total amount with interest
    const monthlyPayment = totalAmount / termMonths;
    const [result] = await pool.query(
      `INSERT INTO loans 
        (memberID, userID, amount, reason, termMonths, interestRate, paymentFrequency, monthlyPayment, remainingBalance, status, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW(), NOW())`,
      [memberID, userID, amount, reason, termMonths, interestRate, paymentFrequency || 'monthly', monthlyPayment, totalAmount] // ✅ initialize remainingBalance = total amount with interest
    );
  ...
  async computeStats() {
    // pending, active, totalLoanAmount, overdue - FIXED CALCULATIONS
    const sql = `
      SELECT
        ...
        IFNULL(SUM(CASE WHEN l.status IN ('approved','active','disbursed') THEN l.amount ELSE 0 END),0) AS totalLoanAmount,
