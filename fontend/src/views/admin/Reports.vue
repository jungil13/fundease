<template>
  <div class="p-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="h-64">
          ...
          <FinancialChart v-else :data="fundGrowthData" :type="chartType1" :height="256" />
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="h-64">
          ...
          <FinancialChart v-else :data="contributionWithdrawalData" :type="chartType2" :height="256" />
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="h-64">
          ...
          <PieChart v-else :data="transactionDistributionData" :height="256" title="Monthly Transaction Types" />
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="h-64">
          ...
          <PieChart v-else :data="fundAllocationData" :height="256" title="Current Fund Allocation" />
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">Weekly Contribution Trend</h3>
      <div class="h-64">
        <div v-if="loading" class="h-full bg-gray-50 rounded-lg flex items-center justify-center">
          <div class="text-center">
            <div class="h-12 w-12 bg-gray-200 rounded animate-pulse mx-auto mb-2"></div>
        ...
        <FinancialChart v-else :data="weeklyContributionsData" :type="'line'" :height="256" />
      </div>
    </div>

    <!-- Weekly Contribution Report -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">Weekly Contribution Report</h3>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Contributions
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Amount
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Active Members
              </th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else-if="weeklyContributions.length === 0" class="hover:bg-gray-50">
              <td colspan="5" class="px-6 py-4 text-center text-gray-500">
                No weekly contribution data available
              </td>
            </tr>
            <tr v-else v-for="week in weeklyContributions" :key="week.week" class="hover:bg-gray-50">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-blue-600">
                ₱{{ week.totalAmount.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                {{ week.memberCount }} members
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
      ...
      <div class="flex flex-wrap gap-4">
        <button @click="exportReport('excel')" :disabled="loading"
        ...
        <button @click="exportReport('pdf')" :disabled="loading"
        ...
        <button @click="window.print()" :disabled="loading"
...
// Weekly contributions chart data
const weeklyContributionsData = computed(() => {
  if (!weeklyContributions.value || weeklyContributions.value.length === 0) {
    return {
      labels: [],
      contributions: [],
      loans: [],
      loansLabel: 'Contributions'
    }
  }

  // Reverse to show oldest to newest
  const sortedWeeks = [...weeklyContributions.value].reverse()

  return {
    labels: sortedWeeks.map(week => `Week ${week.weekNumber}`),
    contributions: sortedWeeks.map(week => week.totalAmount),
    loans: [],
    loansLabel: 'Weekly Contributions (₱)'
  }
})
...
const fetchReportsData = async () => {
  try {
    ...
    if (response.data.success) {
      ...
      weeklyContributions.value = data.weeklyContributions || []
