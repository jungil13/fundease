...
const fetchContributions = async () => {
  try {
    ...
    Swal.fire({
      icon: 'error',
      title: 'Error',
      text: err.response?.data?.message || 'Failed to fetch contributions',
...
const updateStatus = async (contributionID, newStatus, notes = '') => {
  try {
    ...
    const result = await Swal.fire({
      title: newStatus === 'confirmed' ? 'Approve Contribution?' : 'Reject Contribution?',
      ...
      confirmButtonText: newStatus === 'confirmed' ? 'Yes, Approve' : 'Yes, Reject',
    ...
    const response = await api.patch(`/contributions/${contributionID}/status`, {
      status: newStatus,
      notes: notes || ''
    })
    ...
    if (response.data.success) {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Success',
        text: `Contribution ${newStatus === 'confirmed' ? 'approved' : 'rejected'} successfully`,
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <div class="flex space-x-2">
                  ...
                  <div v-if="contribution.status === 'pending'" class="flex space-x-1">
                    <button 
                      @click="updateStatus(contribution.contributionID, 'confirmed')"
                      :disabled="loading"
                      class="text-green-600 hover:text-green-900 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                      Approve
                    </button>
                    <button 
                      @click="updateStatus(contribution.contributionID, 'rejected')"
                      :disabled="loading"
