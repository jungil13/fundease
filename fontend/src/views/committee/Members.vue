...
const viewTransactions = async (member) => {
  ...
  try {
    ...
    // Handle both old format (array) and new format (object with transactions and summary)
    if (Array.isArray(response.data)) {
      transactionHistory.value = response.data
    } else {
      transactionHistory.value = response.data.transactions || []
      transactionSummary.value = response.data.summary || {
        totalContributions: 0,
        totalLoans: 0,
        totalRepayments: 0
      }
        ...
        <div class="p-6 overflow-y-auto max-h-[65vh]">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-green-50 border border-green-200 rounded-lg p-4">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-green-700">Total Contributions</p>
                  <p class="text-2xl font-bold text-green-900 mt-1">{{ formatCurrency(transactionSummary.totalContributions) }}</p>
                </div>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-blue-700">Total Loans</p>
                  <p class="text-2xl font-bold text-blue-900 mt-1">{{ formatCurrency(transactionSummary.totalLoans) }}</p>
                </div>
                <div class="text-3xl">💳</div>
              </div>
            </div>
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-purple-700">Total Repayments</p>
                  <p class="text-2xl font-bold text-purple-900 mt-1">{{ formatCurrency(transactionSummary.totalRepayments) }}</p>
                </div>
                <div class="text-3xl">✅</div>
              </div>
            </div>
