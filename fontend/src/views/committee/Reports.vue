<template>
  <div class="space-y-6">
    ...
    <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl p-6 mb-6">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-if="weeklyContributions.length === 0" class="hover:bg-gray-50">
              <td colspan="5" class="px-6 py-4 text-center text-gray-500">
                No weekly contribution data available
              </td>
            </tr>
            <tr v-else v-for="week in weeklyContributions" :key="week.week" class="hover:bg-purple-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
...
const fetchWeeklyContributions = async () => {
  try {
    loading.value = true
    const response = await api.get('/reports', { params: { period: 'month' } })
    
    if (response.data.success && response.data.data.weeklyContributions) {
      weeklyContributions.value = response.data.data.weeklyContributions
    }
  } catch (err) {
    console.error('Error fetching weekly contributions:', err)
  } finally {
    loading.value = false
  }
}
